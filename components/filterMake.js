...
export default function FilterMake() {
    ...
    return (
        <div className="relative inline-block">
            ...
            <div
                className={`${visible ? '' : 'hidden'} z-50 origin-top-left absolute left-0 mt-1 border-4 border-green-700 bg-white text-center dark:bg-black`}>
                    ...
                    {makes.map(x => {
                        return (
                            <p key={x}
                               ...
                               className="flex flex-row block px-1 py-1 text-sm text-gray-700 hover:bg-gray-200 hover:text-gray-900 dark:text-neutral-100 dark:hover:bg-neutral-700 dark:hover:text-white"
                               role="menuitem">
                                <svg
                                    className={`${prm_makes && prm_makes.some(y => y === x) ? 'text-black dark:text-neutral-100' : 'text-transparent'} w-4 mr-2`}
